...
{ private int numSlots = 0, cont = 0 ,cint = 0,nume=0;   //eliminar cin num
  private double[] buffer = null;
 

   private Condition Puede_quitar = makeCondition();
   private Condition Puede_añadir = makeCondition();
  ...
  { enter();
    if ( nume == numSlots )   Puede_añadir.await();              

    buffer[cint] = valor;   
    cint=(cint+1)%numSlots;  //cont++;bufer[cont]=valor
    nume++;
    Puede_quitar.signal();  
    leave();}
  ...
  { enter();
    ...
    while( nume == 0 ) Puede_quitar.await() ;

    valor = buffer[cont];
    cont=(cont+1)%numSlots;//elimaino linea
    nume--;     //cont-- elimino num
    Puede_añadir.signal();
    leave();
    return valor;
