...
void consumidor(int rank)
{
    ...
    for(unsigned int i = 0; i < 5; i++)
    {
        ...
        MPI_Recv (&value, 1, MPI_INT, Buffer, 0, MPI_COMM_WORLD, &status);  // La etiqueta de envio de buffer a consumidor es 0
    ...
    for(unsigned int i = 0; i < 40; i++)
    {
        ...
        switch(rama)
        {
            case 0 : // Recibe del productor
            {
                ...
                cout << "Buffer recibe " << value[pos] << " de el Productor " << status.MPI_SOURCE << endl  << flush;
                pos++;
                break;
            }
            case 1 : // Envia al consumidor
            {
                ...
                cout  << "Buffer envia " << value[pos-1] << " al Consumidor " << status.MPI_SOURCE << endl << flush;
                pos--;
                break;
            }
        }
